...
@Service
public class SyslogFileStorageService {

    private final Path rootLocation;

    // No FTP configuration needed - using local storage only

    public SyslogFileStorageService(@Value("${file.upload-dir:uploads}") String uploadDir) throws IOException {
    ...
    public String storeInFolder(MultipartFile file, String preferredFolderName) throws IOException {
        ...
        Path targetDir = this.rootLocation.resolve("upload_syslog").resolve(safeFolder);
        ...
        return "/files/upload_syslog/" + safeFolder + "/" + filename;
    }
